import { Component, OnInit, OnChanges, Input, ViewChild, TemplateRef, Output, EventEmitter } from '@angular/core';
...
@Component({
  selector: 'app-bin-vendor-invoice-detail',
  templateUrl: './bin-vendor-invoice-detail.component.html',
  styleUrls: ['./bin-vendor-invoice-detail.component.scss']
})
export class BinVendorInvoiceDetailComponent implements OnInit, OnChanges {
  @Input() invoiceID: any;
  ...
  createNewRowData() {
    ...
    const newData = {
      ...
      invoiceChargeTypeID: 0, invoiceID: 0, invoiceChargeDescription: '', isSaveRequired: true,
    };
    return newData;
  }
    ...
    this._setupService.postData(`InvoiceDetail/GetItemWithLastVendorPrice`, postData)
      .subscribe(res => {
        ...
        if (res && res[0]) {
          ...
          this.unitsInCaseOldValue = res[0].unitsInCase;
          this.invoiceDetailsForm.get('sequenceNumber').setValue(this.sequenceNumber);
          this.invoiceDetailsForm.get('invoiceValuePrice').setValue(res[0].inventoryValuePrice);
  ...
  saveInvoicePaymentDetails(params) {
    ...
    const descriptionObj = _.find(this.commonService.invoicePaymentList, ['sourceName', params.data.sourceName]);
    ...
    //  const descriptionObj = params.data && params.data.sourceName && params.data.sourceName.split(',');
    const postData = {
  ...
  getPaymentSource() {
    this._setupService.getData('PaymentSource/getListByCompanyID/' + this.userInfo.companyId).subscribe(res => {
      if (res && (res['statusCode'])) {
  ...
  calculateUnitPerCostFrm() {
    ...
    // if (cost > 0) {
    //   // this.invoiceDetailsForm.get('regularSellPrice').setValue(buyingCost.toFixed(3));
    //   this.invoiceDetailsForm.get('regularSellPrice').setValue(Number(buyingCost.toFixed(3)));
    // }
    // this.invoiceDetailsForm.get('totalItemCost').setValue(Number(ItemCost.toFixed(3)));
  ...
  calculateBuyingUnitQtyFrm() {
    ...
    if (Number(this.invoiceDetailsForm.get('buyingCaseQuantity').value) <= 0 &&
      ...
      let x = Number(this.invoiceDetailsForm.get('buyingUnitQuantity').value) *
        ...
        if (!isNumber(x)) { x = 0; }
      this.invoiceDetailsForm.get('totalItemCost').setValue(x.toFixed(3));
    }
  }
  calculateMarginFrm() {
    ...
    if (this.invoiceDetailsForm.get('sellingUnits').value !== '') {
      rSellPrice = this.invoiceDetailsForm.get('sellingUnits').value;
    }
    ...
    if (this.invoiceDetailsForm.get('sellingUnits').value === 0) {
      ...
      this.invoiceDetailsForm.get('sellingUnits').setValue(Number(z.toFixed(3)));
    }
  ...
  calculateSelling() {
    ...
    txtProfitMargin = this.invoiceDetailsForm.get('profitMargin').value;
    ...
    if (this.invoiceDetailsForm.get('profitMargin').value !== '') {
      profitMargin = this.invoiceDetailsForm.get('profitMargin').value;
    }
    if (this.invoiceDetailsForm.get('casePrice').value !== '') {
      unitCostPrice = this.invoiceDetailsForm.get('casePrice').value;
    }
    ...
    if (z > 0) {
      // App.txtRegularSellingPrice.setValue(z.toFixed(3));
      this.invoiceDetailsForm.get('sellingUnits').setValue(z.toFixed(3));
    } else {
  ...
  refreshSellingPrice() {
    ...
    const storeLocationItemIDs = this.selectedRows.map(x => x.storeLocationItemID).join(',');
    const invoiceDetailIDs = this.selectedRows.map(x => x.invoiceDetailID).join(',');
    this.spinner.show();
    // tslint:disable-next-line:max-line-length
  ...
  calculateCaseCost() {
    ...
    this.invoiceDetailsForm.get('casePrice').setValue(Number(caseCost).toFixed(3));
  ...
  calculateTotalCost() {
    ...
    //   this.invoiceDetailsForm.get('totalItemCost').setValue(Number(totalCost).toFixed(3));
    // } else {
  ...
  totalCostChange() {
    ...
    if (Number(this.invoiceDetailsForm.get('buyingCaseQuantity').value) === 0 &&
      ...
      this.invoiceDetailsForm.get('casePrice').setValue(dos.toFixed(3));
    }
    if ((Number(this.invoiceDetailsForm.get('buyingCaseQuantity').value) >= 0 ||
      Number(this.invoiceDetailsForm.get('buyingCaseQuantity').value) < 0) &&
      Number(this.invoiceDetailsForm.get('buyingUnitQuantity').value) === 0) {
      ...
      if (!isNumber(caseCost)) { caseCost = 0; }
      this.invoiceDetailsForm.get('casePrice').setValue(caseCost.toFixed(3));
  ...
  calculateMargin() {
    ...
    const buyDown = this.invoiceDetailsForm.get('buyDown').value;
  ...
  setInvoiceAmount() {
    ...
    const _invoiceChargeAmount = _.sumBy(this.invoiceChargesGridRowData, 'invoiceChargeAmount');
    const _itemCost = _.sumBy(this.rowData, 'itemCost');
    const amountPaid = _.sumBy(this.invoicePaymentGridRowData, 'amountPaid');
    const sum = _invoiceChargeAmount + _itemCost;
    this.changeInvoiceAmt.emit({
      _invoiceChargeAmount: _invoiceChargeAmount.toFixed(2),
  ...
  onInvoiceGridColHideShow(isShow) {
    this.gridColumnApi.setColumnsVisible(['vendorItemCode', 'checkBox', 'departmentDescription',
      'unitsInCase', 'profitMargin'
    ], isShow);
  }
  openSoftCopy() {
    this.openPDF();

...
export class SelectRenderer implements ICellRendererAngularComp {
    ...
    OnChange(event) {
        this.params.context.componentParent.onChange(parseInt(this.params.node.id), this.params.cellfieldId, event.target.value, this.params.textfield, event.target.options[event.target.options.selectedIndex].text, this.params);
    }
}